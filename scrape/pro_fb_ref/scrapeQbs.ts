...
export class PfrQbScrape extends PfrTools {
  public year = 2017;
  ...
  public scrapeSeasonData(playerId: string): Promise<Array<IPfrSeasonScrape>> {
    const basePlayerUrl = `https://www.pro-football-reference.com/players/${playerId.charAt(0)}/${playerId}.htm#all_rushing_and_receiving`;
  ...
  public async parseSeaonData() {
    let filter = {
      where: {
        position: 'QB'
      }
    };
    let players = await this.getAllPfrPlayers(filter);
    let response = await this.scrapeSeasonData(players[0].pfrId.toString());
  }
